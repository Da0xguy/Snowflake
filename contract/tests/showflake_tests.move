...
fun test_minting() {
    ...
    showflake::mint(ctx).destroy_yeti();
}
...
fun test_upgrade() {
    ...
    let mut yeti = showflake::mint(ctx);
...
fun test_only_one_nft_per_address() {
    ...
    let first = showflake::mint_v2(&mut registry, ctx);

    // second mint from same context/address should fail (registry enforces one NFT per address)
    let second = showflake::mint_v2(&mut registry, ctx);
